...
//!
//! WHY NOT ON THE HOST. btrfs-progs on a workstation is whatever that
//! machine happens to have: a Homebrew formula on a Mac, a distribution
...
//! btrfs-progs at all, and every machine gets the same answers.
//!
...
/// hands `btrfs check` the path it used on the host —
/// `<repo>/tmp/x.img` — so the guest is given that same absolute path
/// as a symlink to the mount. Every argument then crosses unchanged: no
/// rewriting of arguments, none of the paths inside a `btrfs
/// inspect-internal` invocation, and no copying of images in and out.
///
...
///
/// ```ignore
/// let out = oracle("btrfs").args(["check", "--readonly", &image]).output();
...
/// Start building a call to `tool` (`mkfs.btrfs`, `btrfs`,
/// `btrfs-image`, `setfattr`, `getfattr`, `chattr`, `lsattr`).
///
...
impl Arg {
    ...
    /// the tests that check that hand `getfattr` exactly such names.
    #[track_caller]
...
impl Oracle {
    ...
    /// Bytes on the tool's standard input.
    ...
    fn script(&self, run: &Run) -> String {
        ...
        // A tool's own exit status is data here — `btrfs check` answers
        // 1 for "errors found", and the harness answers 1 for "no VM" — so it
    ...
    fn check_path(&self, argument: &str) {
        ...
        assert!(
            ...
            "`{}` was given {argument}, which is outside {}. The oracle tools run in \
             ...
             fs_btrfs_test_support::temp_dir() (`temp_path!`), which is inside the \
