...
fn runs_covering_the_library_unit_tests(workflow: &str) -> Vec<String> {
    workflow
        .lines()
        .filter_map(|raw| {
...
///
/// A `run:` whose value is a `|` block is joined into one string, so
/// the `for t in ...; do cargo test --test "$t"` loops in this
/// repository's `kernel-gate` job are seen whole rather than as
/// fragments. There are TWO such loops; matching the loop statement
/// rather than a line range is what keeps the second one visible.
#[derive(Debug)]
struct Step {
    keys: Vec<String>,
    run: String,
...
fn debug_runs_that_prove_the_build_traps(workflow: &str) -> Vec<String> {
    runs_covering_the_library_unit_tests(workflow)
        .into_iter()
...
";
    assert!(
        runs_covering_the_library_unit_tests(kernel_gate_shapes).is_empty(),
    ...
    // must be counted.
    let all_targets = "      - run: EXPECT_OVERFLOW_CHECKS=1 cargo test --locked --all-targets\n";
    assert_eq!(
...
";
    assert_eq!(
        runs_covering_the_library_unit_tests(release_yml_as_it_is),
        vec!["- run: cargo test --all-targets".to_string()],
...
";
        assert_eq!(
            runs_covering_the_library_unit_tests(quoted_in_a_comment),
            Vec::<String>::new(),
    ...
    #[test]
    fn a_real_library_covering_debug_run_counts() {
        let with_the_step = "\
jobs:
  test:
    steps:
...
";
        assert_eq!(
            runs_covering_the_library_unit_tests(with_the_step),
    ...
    fn a_single_integration_target_is_not_the_library() {
        let lines = [
            "      - run: cargo test --test csum_oracle -- --nocapture",
            "      - run: cargo test --locked --test transaction_oracle",
            "      - run: EXPECT_OVERFLOW_CHECKS=1 cargo test --locked --test capi",
        ];
        for line in lines {
            assert_eq!(
                runs_covering_the_library_unit_tests(line),
    ...
    fn a_cargo_build_step_is_not_a_test_run() {
        let build_only = "      - run: cargo build --locked --release\n";
        assert_eq!(
