...
#
# The build itself lives in scripts/build-nodatacow-fixtures.sh, which
# CI runs directly. It used to be inline here, and being inline is why
# no workflow could run it: the image existed on developers' machines
# and nowhere else, so tests/csum_oracle.rs skipped in CI and reported
# success on every branch.
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# The build lives in scripts/build-compression-fixtures.sh, which CI runs
# directly (#69); it is copied into the guest with the geometry list it
# reads.
wanted_algos=()
for algo in $BTRFS_COMPRESSION_ALGOS; do
    if wanted "comp-$algo"; then wanted_algos+=("$algo"); fi
done
if [ "${#wanted_algos[@]}" -gt 0 ]; then
    guest_path="$("$REPO/scripts/vm.sh" put "$REPO/scripts/build-compression-fixtures.sh")"
    "$REPO/scripts/vm.sh" put "$REPO/scripts/fixture-geometries.sh" >/dev/null
    "$REPO/scripts/vm.sh" run "BTRFS_FIXTURE_DIR=/share bash '$guest_path' ${wanted_algos[*]}"
    rm -f "$REPO/.vm-share/build-compression-fixtures.sh" "$REPO/.vm-share/fixture-geometries.sh"
fi
