...
# the host itself and this is not used. On a Mac there is no btrfs, no
# btrfs-progs worth trusting and no loop mount — so the suite runs in here,
...
export CARGO_TARGET_DIR=/var/cache/fs-btrfs-target
