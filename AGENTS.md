# Working in rust-fs-btrfs (agent guide)

Pure-Rust Btrfs driver exposing a C ABI, validated against btrfs-progs and the in-kernel Btrfs driver inside the harness VM. This file is the fast path
...
```sh
chore test         # everything CI runs
chore testunit     # no fixtures, no tools
chore fixtures     # build the kernel-made images
chore testoracle   # against btrfs-progs
chore testkernel   # against the in-kernel driver
...
`mkfs.btrfs`, `btrfs check` and **the in-kernel Btrfs driver**. The kernel is the
one that matters: our reader and our writer share an interpretation, so only a
real kernel mount can catch the case where both agree with each other and
disagree with every real filesystem.

Everything Linux happens **inside the harness VM**, not on the runner. Fixtures
built by the runner's `mkfs.btrfs` and mounted against the runner's kernel were
graded by a different oracle than a developer's, which is how two halves of "the
same suite" came to disagree. There is one oracle now and it is the guest's.

`tests/kernel_readback.rs` is the pattern worth copying: the kernel reads back
